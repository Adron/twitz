...
func exportParsedTwitterList(exportFilename string, exportFormat string, twittererList []string) {
	if exportFormat == "txt" {
		twitTwitz.ExportTxt(exportFilename, twittererList, exportFormat)
	} else if exportFormat == "json" {
		twitTwitz.ExportJson(exportFilename, twittererList, exportFormat)
	} else if exportFormat == "xml" {
		twitTwitz.ExportXml(exportFilename, twittererList, exportFormat)
	} else if exportFormat == "csv" {
		twitTwitz.ExportCsv(exportFilename, twittererList, exportFormat)
	} else {
